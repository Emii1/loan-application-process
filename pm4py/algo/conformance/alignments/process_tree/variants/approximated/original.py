...
def __approximate_alignment_for_trace(
    ...
    try:
        if len(trace) <= max_tl or get_process_tree_height(pt) <= max_th:
            return calculate_optimal_alignment(
                pt, trace, parameters=parameters
            )
        else:
            if pt.operator == Operator.SEQUENCE:
                return __approximate_alignment_on_sequence(
                    pt,
                    trace,
                    a_sets,
                    sa_sets,
                ...
                return __approximate_alignment_on_loop(
                    pt,
                    trace,
                    a_sets,
                    sa_sets,
                ...
                return __approximate_alignment_on_choice(
                    pt,
                    trace,
                    a_sets,
                    sa_sets,
                ...
                return __approximate_alignment_on_parallel(
                    pt,
                    trace,
                    a_sets,
                    sa_sets,
...
def __approximate_alignment_on_loop(
    ...
    ilp += (
        lpSum(
            [
                s_variables[i][j] * s_costs[i][j]
                for i in range(len(trace))
                for j in range(len(pt.children))
            ]
            + [
                e_variables[i][j] * e_costs[i][j]
                for i in range(len(trace))
                for j in range(len(pt.children))
            ]
            + [
                v_variables[i][j] * v_costs[i][j]
                for i in range(len(trace))
                for j in range(len(pt.children))
            ]
            + [
                t_variables[i][j] * t_costs[i][j]
                for i in range(len(trace) + 1)
                for j in range(len(pt.children))
            ]
        ),
    ...
    for i in range(len(trace)):
        ...
        ilp += (
            lpSum([x_variables[i][j] * 1 for j in range(len(pt.children))])
            == 1
        )

        # start/end/intermediate-activity at position i can only be assigned to one subtree
        ilp += (
            lpSum([s_variables[i][j] * 1 for j in range(len(pt.children))])
            <= 1
        )
        ilp += (
            lpSum([e_variables[i][j] * 1 for j in range(len(pt.children))])
            <= 1
        )
        ilp += (
            lpSum([v_variables[i][j] * 1 for j in range(len(pt.children))])
            <= 1
        )
    ...
    for i in range(len(trace) - 1):
        ilp += (
            n_variables[i][0] <= x_variables[i + 1][1] + t_variables[i + 1][1]
        )
        ilp += n_variables[i][0] >= x_variables[i + 1][1]
        ilp += n_variables[i][0] >= t_variables[i + 1][1]

        ilp += (
            n_variables[i][1] <= x_variables[i + 1][0] + t_variables[i + 1][0]
        )
    ...
    for i in range(len(trace)):
        for j in range(2):
            ...
            ilp += (
                e_variables[i][j] >= n_variables[i][j] + x_variables[i][j] - 1
            )
...
def __approximate_alignment_on_sequence(
    ...
    for i in range(1, len(trace)):
        for j in range(len(pt.children)):
            ilp += (
                s_variables[i][j]
                >= x_variables[i][j] + 1 - x_variables[i - 1][j] - 1
            )
    ...
    for i in range(len(trace) - 1):
        for j in range(len(pt.children)):
            ilp += (
                lpSum(
                    x_variables[i + 1][k] for k in range(j, len(pt.children))
                )
                >= x_variables[i][j]
            )
    ...
    for j in range(len(pt.children)):
        ...
        ilp += u_variables[j] <= lpSum(
            s_variables[i][j] for i in range(len(trace))
        )
        # if u_j variable = 1 ==> an end activity must exist
        ilp += u_variables[j] <= lpSum(
            e_variables[i][j] for i in range(len(trace))
        )
