...
def get_hidden_transitions_to_enable(
    marking, places_with_missing, places_shortest_path_by_hidden
):
    """
    ...
    for p1 in marking_places:
        for p2 in places_with_missing_keys:
            if (
                p1 in places_shortest_path_by_hidden
                and p2 in places_shortest_path_by_hidden[p1]
            ):
...
def get_req_transitions_for_final_marking(
...
def apply_trace(
    ...
    for i in range(len(trace)):
        if enable_postfix_cache and (
            ...
            if enable_marktoact_cache and (
                ...
                if trace[i][activity_key] in trans_map:
                    ...
                    corr_en_t = [
                        ...
                        if x.label == trace[i][activity_key]
                    ]
                    if corr_en_t:
                        t = corr_en_t[0]
                    else:
                    ...
                    if walk_through_hidden_trans and not semantics.is_enabled(
                        ...
                        prev_len_activated_transitions = len(act_trans)
                        [net, new_marking, new_act_trans, new_vis_mark] = (
                            apply_hidden_trans(
                                t,
                                net,
                                copy(marking),
                    ...
                    if not semantics.is_enabled(t, net, marking):
                        ...
                        if enable_pltr_fitness:
                            for place in tokens_added.keys():
                                ...
                                place_fitness[place]["m"] += tokens_added[
                                    place
                                ]
                            if (
                                trace
            ...
            if (
                len(trace_activities)
                < TechnicalParameters.MAX_POSTFIX_SUFFIX_LENGTH.value
            ):
            ...
            if i > 0:
                activating_transition_interval.append(
                    [
                        trace[i][activity_key],
                        prev_len_activated_transitions,
                        len(act_trans),
    ...
    if enable_pltr_fitness:
        ...
        for pl in diff_fin_mark_mark:
            place_fitness[pl]["m"] += (
                diff_fin_mark_mark[pl] * trace_occurrences
            )

    if consumed > 0 and produced > 0:
        trace_fitness = 0.5 * (
            1.0 - float(missing) / float(consumed)
        ) + 0.5 * (1.0 - float(remaining) / float(produced))
    else:
        trace_fitness = 1.0
    ...
    if is_fit:
        for suffix in activating_transition_index:
            ...
            if (
                ...
                not in post_fix_caching.cache[suffix]
            ):
                post_fix_caching.cache[suffix][
                    activating_transition_index[suffix]["marking"]
                ] = {
        ...
        for trans in activating_transition_interval:
            ...
            if end_marking_index < len(vis_mark):
                ...
                if activity in trans_map:
                    ...
                    if (
                        start_marking_hash
                        not in marking_to_activity_caching.cache
                    ):
...
def apply(
    ...
    disable_variants = exec_utils.get_param_value(
        Parameters.DISABLE_VARIANTS, parameters, enable_pltr_fitness
    )
    return_names = exec_utils.get_param_value(
        Parameters.RETURN_NAMES, parameters, False
    )
    thread_maximum_ex_time = exec_utils.get_param_value(
        Parameters.THREAD_MAX_EX_TIME,
        parameters,
        TechnicalParameters.MAX_DEF_THR_EX_TIME.value,
    )
    places_shortest_path_by_hidden = exec_utils.get_param_value(
        Parameters.PLACES_SHORTEST_PATH_BY_HIDDEN, parameters, None
    )
