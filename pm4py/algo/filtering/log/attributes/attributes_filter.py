...
def apply_numeric(
    ...
    case_attribute_prefix = exec_utils.get_param_value(
        Parameters.PARAMETER_KEY_CASE_GLUE,
        parameters,
    ...
    if stream_filter_key1 is not None:
        stream = EventStream(
            list(
                filter(
                    lambda x: stream_filter_key1 in x
                    and x[stream_filter_key1] == stream_filter_value1,
                    stream,
                )
            ),
    ...
    if stream_filter_key2 is not None:
        stream = EventStream(
            list(
                filter(
                    lambda x: stream_filter_key2 in x
                    and x[stream_filter_key2] == stream_filter_value2,
                    stream,
                )
            ),
            attributes=log.attributes,
    ...
    if positive:
        stream = EventStream(
            list(
                filter(
                    lambda x: attribute_key in x
                    and int1 <= x[attribute_key] <= int2,
                    stream,
                )
            ),
            attributes=log.attributes,
        ...
        stream = EventStream(
            list(
                filter(
                    lambda x: attribute_key in x
                    and (x[attribute_key] < int1 or x[attribute_key] > int2),
                    stream,
                )
            ),
            attributes=log.attributes,
...
def apply_trace_attribute(
    ...
    for trace in log:
        if positive:
            if (
                attribute_key in trace.attributes
                and trace.attributes[attribute_key] in values
            ):
...
def filter_log_by_attributes_threshold(
    log,
    attributes,
    variants,
    vc,
...
def filter_log_relative_occurrence_event_attribute(
    ...
    activities_occurrences = get_attribute_values(
        log, attribute_key, parameters=parameters_cp
    )
