...
def sublog_percent(log, upper_percent, parameters=None):
    ...
    if parameters is None:
        parameters = {}
    lower_percent = exec_utils.get_param_value(
        Parameters.LOWER_PERCENT, parameters, 0
    )
...
def logslice_percent_act(log, unit):
    ...
    for i in range(len(num_list)):
        (df, act_list) = sublog_percent2actlist(
            log, num_list[i] + unit, parameters={"lower_percent": num_list[i]}
        )
        if len(act_list) != 0:
...
def logslice_percent(log, unit):
    ...
    for i in range(len(num_list)):
        (df, var_list) = sublog_percent2varlist(
            log, num_list[i] + unit, parameters={"lower_percent": num_list[i]}
        )
        if len(var_list) != 0:
            log1 = apply_variants_filter(
                log, var_list, parameters={"positive": True}
            )
            sum1 = np.array(df["count"]).sum()
            loglist.append(log1)
            freq_list.append(sum1)
    ...
    for i in range(max_len):
        if i < min_len:
            for j in range(0, i + 1):
                ...
                df_1 = pandas_utils.instantiate_dataframe_from_dict(
                    dict(result), orient="index", columns=["freq_1"]
                )  # convert dict to dataframe
                df_1 = df_1.reset_index().rename(columns={"index": "var"})
                result = Counter(
                    min_var[j]
                )  # count number of occurrence of each element
                df_2 = pandas_utils.instantiate_dataframe_from_dict(
                    dict(result), orient="index", columns=["freq_2"]
                )
                df_2 = df_2.reset_index().rename(columns={"index": "var"})
                df = pandas_utils.merge(
                    df_1, df_2, how="outer", on="var"
                ).fillna(
                    0
                )  # merge two variants and replace empty value by zero
                df["prod"] = df.apply(
                    lambda x: x["freq_1"] * x["freq_2"], axis=1
                )
